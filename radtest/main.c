...
static int
optset_assign(struct parseopt *po, struct optdef *opt, char *arg)
{
	assign(arg);
	return 0;
}
...
static int
optset_verbose(struct parseopt *po, struct optdef *opt, char *arg)
{
	verbose = 1;
...
static int
optset_filename(struct parseopt *po, struct optdef *opt, char *arg)
{
	filename = arg;
	po->po_eopt = 1;
	return 0;
...
static struct optdef options[] = {
	{
		...
		.opt_doc = N_("radtest specific switches:"),
	},

	{
		.opt_name = "assign",
		.opt_doc = N_("assign a VALUE to VARIABLE"),
		.opt_argdoc = N_("VARIABLE=VALUE"),
		.opt_set = optset_assign
	},
	{
		.opt_name = "a",
	...
	{
		.opt_name = "file",
		.opt_doc = N_("read input from FILE; stops further processing of the command line"),
		.opt_argdoc = N_("FILE"),
		.opt_set = optset_filename
	},
	{
		.opt_name = "f",
		.opt_flags = OPTFLAG_ALIAS
	...
	{
		.opt_name = "quick",
		.opt_doc = N_("don't read the configuration file"),
		.opt_set = optset_true,
		.opt_ptr = &quick
	},
	{
	...
	{
		.opt_name = "retry",
		.opt_doc = N_("set number of retries"),
	...
	{
		.opt_name = "server",
		.opt_doc = N_("set radius server parameters"),
	...
	{
		.opt_name = "verbose",
		.opt_doc = N_("verbose mode"),
		.opt_set = optset_verbose
	},
	{
		.opt_name = "v",
		.opt_flags = OPTFLAG_ALIAS
	...
	{
		.opt_name = "no-interactive",
		.opt_doc = N_("disable interactive mode"),
		.opt_set = optset_true,
		.opt_ptr = &disable_readline
	},
	{
		.opt_name = "i",
		.opt_flags = OPTFLAG_ALIAS
	...
	{
		.opt_name = "dry-run",
		.opt_doc = N_("Check the input file syntax and exit"),
		.opt_set = optset_true,
		.opt_ptr = &dry_run
	},
	{
...
static struct parseopt po = {
	.po_descr = N_("Radius client shell"),
	.po_optdef = optdef,
	.po_flags = PARSEOPT_IN_ORDER
};

int
radtest_parse_options(int argc, char **argv)
{
	po.po_flags = PARSEOPT_NO_ERREXIT;
...
main(int argc, char **argv)
{
	...
	if (server) {
		...
		if (argc < 4)
			serv.port[0] = RADIUS_AUTH_PORT;
		if (argc < 6)
			serv.port[1] = RADIUS_ACCT_PORT;
