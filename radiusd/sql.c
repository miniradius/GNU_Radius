...
static char *get_conf_line(void);
...
get_conf_line(void)
{
	...
	if (cur_line)
		grad_strbuf_free_string(parse_sb, cur_line);
	cont = 1;
	while (cont) {
		...
		/* add to the stack */
		grad_strbuf_grow(parse_sb, ptr, eff_len);
	}

	if (total_length == 0)
		return NULL;
	cur_ptr = cur_line = grad_strbuf_finish(parse_sb, 0);
	/* recognize keyword */
...
sql_cache_retrieve(struct sql_connection *conn, char *query)
{
	...
	res->tuple = grad_ecalloc(res->ntuples, sizeof(res->tuple[0]));
	for (i = 0; i < res->ntuples
		     && disp_sql_next_tuple(conn, data) == 0; i++) {
		int j;

		res->tuple[i] = grad_ecalloc(res->nfields,
					     sizeof(res->tuple[0][0]));
...
detach_sql_connection(int type)
{
	...
	if (conn->destroy_on_close) {
		GRAD_DEBUG(1, "destructing sql connection %p", conn);
...
radiusd_sql_acct(radiusd_request_t *radreq)
{
	...
	case DV_ACCT_STATUS_TYPE_START:
		...
		if (!sql_cfg.query[acct_start_query])
			break;
		query = radius_xlate(sql_cfg.query[acct_start_query],
				     radreq->request, NULL);
	...
	case DV_ACCT_STATUS_TYPE_STOP:
		...
		if (!sql_cfg.query[acct_stop_query])
			break;
		query = radius_xlate(sql_cfg.query[acct_stop_query],
				     radreq->request, NULL);
	...
	case DV_ACCT_STATUS_TYPE_ACCOUNTING_ON:
		...
		if (!sql_cfg.query[acct_nasup_query])
			break;
		query = radius_xlate(sql_cfg.query[acct_nasup_query],
				     radreq->request, NULL);
		rc = disp_sql_query(conn, query, &count);
		sqllog(rc, query);
		if (rc == 0)
	...
	case DV_ACCT_STATUS_TYPE_ACCOUNTING_OFF:
		...
		if (!sql_cfg.query[acct_nasdown_query])
			break;
		query = radius_xlate(sql_cfg.query[acct_nasdown_query],
				     radreq->request, NULL);
		rc = disp_sql_query(conn, query, &count);
		sqllog(rc, query);
		if (rc == 0)
	...
	case DV_ACCT_STATUS_TYPE_ALIVE:
		...
		if (!sql_cfg.query[acct_keepalive_query])
			break;
		query = radius_xlate(sql_cfg.query[acct_keepalive_query],
				     radreq->request, NULL);
		rc = disp_sql_query(conn, query, &count);
		sqllog(rc, query);
		if (rc == 0 && count != 1)
...
radiusd_sql_pass(radiusd_request_t *req, char *authdata)
{
	...
	query = radius_xlate(sql_cfg.query[auth_query],
			     req->request, NULL);
...
radiusd_sql_checkgroup(radiusd_request_t *req, char *groupname)
{
	...
	query = radius_xlate(sql_cfg.query[group_query], req->request, NULL);
...
radiusd_sql_reply_attr_query(radiusd_request_t *req, grad_avp_t **reply_pairs)
{
	...
	conn = attach_sql_connection(SQL_AUTH);
	query = radius_xlate(sql_cfg.query[reply_attr_query],
			     req->request, NULL);
...
radiusd_sql_check_attr_query(radiusd_request_t *req, grad_avp_t **return_pairs)
{
	...
	conn = attach_sql_connection(SQL_AUTH);
	query = radius_xlate(sql_cfg.query[check_attr_query],
			     req->request, NULL);
...
radiusd_sql_auth_result_query(radiusd_request_t *req, int fail)
{
	...
	conn = attach_sql_connection(SQL_AUTH);
	query = radius_xlate(sql_cfg.query[q], req->request, NULL);
	rc = disp_sql_query(conn, query, NULL);
	sqllog(rc, query);
	free(query);
}
		...
		grad_list_t **sess_list)
{
	...
	conn = attach_sql_connection(SQL_ACCT);
	query = radius_xlate(query_template, request->request, NULL);
...
rad_sql_mlc_close(struct radutmp *up)
{
	...
	grad_avl_add_pair(&req->avlist,
			  grad_avp_create_string(DA_ACCT_SESSION_ID,
						 up->session_id));
	query = radius_xlate(sql_cfg.query[query_index], req, NULL);
