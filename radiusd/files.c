...
{
	if (!user_tab)
		user_tab = grad_symtab_create(sizeof(User_symbol),
					      free_user_entry);
	   ...
	   grad_avp_t **check_pairs, grad_avp_t **reply_pairs)
{
	...
	} while ((sym = grad_sym_next((grad_symbol_t*)sym)) != NULL);
		  ...
		  grad_matching_rule_t *rule)
{
	...
	if ((tmp = grad_avl_find(rule->rhs, DA_REPLACE_USER_NAME))
		...
		char *ptr;
		ptr = radius_xlate(tmp->avp_strvalue, req->request, NULL);
 ...
 */
CLIENT *
client_lookup_ip(uint32_t ipaddr)
{
	CLIENT *cl;
 ...
 */
char *
client_lookup_name(uint32_t ipaddr, char *buf, size_t bufsize)
{
	CLIENT *cl;
	       ...
	       char *stripped_name)
{
	...
	if (presuf_item) {
		tmp = grad_avl_find(check_item, DA_STRIP_USER_NAME);
		if (tmp)
			do_strip = tmp->avp_lvalue;
		if (do_strip) {
