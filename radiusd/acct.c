...
static int write_nas_restart(int status, uint32_t addr);
...
int
radzap(uint32_t nasaddr, int port, char *user, time_t t)
{
	struct radutmp  *up;
	radut_file_t    file;
	uint32_t           netaddr;
		     ...
		     char *default_filename)
{
	if (!template) {
		...
		return radius_xlate(template, req->request, NULL);
	}
}
...
int
write_nas_restart(int status, uint32_t addr)
{
	struct radutmp ut;
