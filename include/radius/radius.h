...
typedef struct {
	uint32_t addr;     /* Server IP address */
...
struct radius_server {
	...
	uint32_t addr;     /* IP address of it */
...
typedef struct {
	uint32_t source_ip; /* Source IP address for xmits */
...
typedef struct grad_request {
	uint32_t ipaddr;       /* Source IP address */
...
void grad_parser_lex_finish(void);
int grad_parser_lex_sync(void);

#define GRAD_NITEMS(a) sizeof(a)/sizeof((a)[0])

size_t grad_create_pdu(void **rptr, int code, int id,
		       u_char *authenticator, char *secret,
		       grad_avp_t *pairlist, char *msg);

grad_request_t *grad_decode_pdu(uint32_t host,
...
grad_dict_value_t *grad_value_lookup(uint32_t, char *);
...
grad_avp_t *grad_avp_create_integer(int attr, uint32_t value);
...
char *grad_ip_gethostname (uint32_t, char *buf, size_t size);
uint32_t grad_ip_gethostaddr (const char *);
char *grad_ip_iptostr(uint32_t, char *);
uint32_t grad_ip_strtoip(const char *);
int grad_ip_getnetaddr(const char *str, grad_netdef_t *netdef);
int grad_ip_in_net_p(const grad_netdef_t *netdef, uint32_t ipaddr);

/* nas.c */
grad_iterator_t *grad_nas_iterator();
int grad_nas_read_file(char *file);
grad_nas_t *grad_nas_lookup_name(char *name);
grad_nas_t *grad_nas_lookup_ip(uint32_t ipaddr);
char *grad_nas_ip_to_name(uint32_t ipaddr, char *buf, size_t size);
...
grad_realm_t *grad_realm_lookup_ip(uint32_t ip);
int grad_read_realms(char *filename, int auth_port, int acct_port,
		     int (*set_secret)(grad_server_t *));
int grad_realm_verify_ip(grad_realm_t *realm, uint32_t ip);
...
void grad_client_random_authenticator(u_char *authenticator);
