...
grad_list_t *grad_list_create(void);
...
#define SLIST_FREE(head, field, type, freefn)			\
	while (!SLIST_EMPTY(head)) {				\
		struct type *__ptr = SLIST_FIRST(head);		\
...
#define DLIST_FREE(head, field, type, freefn)			\
	while (!DLIST_EMPTY(head)) {				\
		struct type *__ptr = DLIST_FIRST(head);		\
		DLIST_REMOVE_HEAD(head, field);			\
		freefn(__ptr);					\
	}
