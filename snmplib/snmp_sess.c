		  ...
		  int local_port, int timeout, int retries)
{
	...
	if (getaddrinfo(sp->remote_host,
			sp->remote_port == 0 ? "snmp" : NULL,
			&hints, &res)) {
		SNMP_SET_ERRNO(E_SNMP_BAD_ADDRESS);
		snmp_session_close(sp);
		return -1;
	}
	sp->remote_sin = *(struct sockaddr_in*)res->ai_addr;
	if (sp->remote_port)
		sp->remote_sin.sin_port = htons(sp->remote_port);
