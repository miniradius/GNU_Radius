...
u_char *
snmp_var_decode(u_char *data, u_int *length, struct snmp_var **var_head,
		int version)
{
	u_char *buf, *tmp;
	u_char *data_ptr;
	u_char type;
	u_int list_length = *length;
	u_int var_length, data_len;
...
u_char *
snmp_var_encode(u_char *data, u_int *length, struct snmp_var *var, int version)
{
	u_char *buf = data;
