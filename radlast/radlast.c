...
static int
optset_digit(struct parseopt *po, struct optdef *opt, char *arg)
{
	unsigned long i, k, n;
...
static int
optset_hostip(struct parseopt *po, struct optdef *opt, char *arg)
{
...
static int
optset_wide(struct parseopt *po, struct optdef *opt, char *arg)
{
	width = 8;
	return 0;
}
...
static struct optdef options[] = {
	{
		...
		.opt_doc = N_("radlast specific switches:"),
	},
	{
		.opt_name = "0",
		.opt_argdoc = "",
		.opt_flags = OPTFLAG_HIDDEN|OPTFLAG_ARG_OPTIONAL,
		.opt_set = optset_digit
	},
	{
		.opt_name = "1",
		.opt_argdoc = "",
	...
	{
		.opt_name = "count",
		.opt_doc = N_("show at most NUMBER records"),
	...
	{
		.opt_name = "file",
		.opt_doc = N_("use FILE as radwtmp"),
		.opt_argdoc = N_("FILE"),
	...
	{
		.opt_name = "host",
		.opt_doc = N_("show logins with IPADDR"),
		.opt_argdoc = N_("IPADDR"),
		.opt_set = optset_hostip
	},
	{
		.opt_name = "h",
		.opt_flags = OPTFLAG_ALIAS
	...
	{
		.opt_name = "missed-stops",
		.opt_doc = N_("mark records with missed stops with bump (!)"),
		.opt_set = optset_true,
		.opt_ptr = &mark_missing_stops
	},
	{
		.opt_name = "m",
		.opt_flags = OPTFLAG_ALIAS
	...
	{
		.opt_name = "nas",
		.opt_doc = N_("show logins from given NAS"),
	...
	{
		.opt_name = "long-format",
		.opt_doc = N_("use long output format"),
		.opt_set = optset_true,
		.opt_ptr = &long_fmt
	},
	{
	...
	{
		.opt_name = "port",
		.opt_doc = N_("show logins from given port"),
		.opt_argdoc = N_("NUMBER"),
		.opt_set = optset_port,
		.opt_ptr = &port
	},
	...
	{
		.opt_name = "show-seconds",
		.opt_doc = N_("show the login session duration in seconds"),
		.opt_set = optset_true,
		.opt_ptr = &show_seconds
	},
	{
		.opt_name = "s",
		.opt_flags = OPTFLAG_ALIAS
	...
	{
		.opt_name = "wide",
		.opt_doc = N_("widen the duration field to show seconds"),
		.opt_set = optset_wide,
	},
	{
		.opt_name = "w",
...
static struct parseopt po = {
	.po_descr = N_("report last logins from Radius database."),
	.po_optdef = optdef,
...
main(int argc, char **argv)
{
	...
	if (nas_name) {
		...
		if (nas) {
			nas_ip = nas->netdef;
		} else if (grad_ip_getnetaddr(nas_name, &nas_ip)) {
		...
		for (bp = &buf[bytes / sizeof(buf[0]) - 1]; !stop && bp >= buf;
			...
			case P_LOGIN:
				if ((pp = find_logout(bp)) != NULL) {
					...
					delete_logout(pp, bp);
				} else if ((pp = find_restart(bp)) != NULL) {
					if (want(bp)) {
						print_entry(pp, bp, 0);
						if (maxrec != -1 && !--maxrec)
							return;
					}
				} else if ((pp = find_login(bp)) != NULL) {
...
add_logout(struct radutmp *bp)
{
	...
	/* purge deleted queue */
	if ((wp = find_wtmp_nas_port(login_list, bp)) != NULL) {
...
add_nas_restart(struct radutmp *bp)
{
	...
	if ((wp = find_wtmp_nas(nas_updown_list, bp)) != NULL) {
