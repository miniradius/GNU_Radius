...
void
grad_client_random_authenticator(u_char *authenticator)
{
	int randno;
	int i;
...
grad_request_t *
grad_client_recv(uint32_t host, u_short udp_port, char *secret,
		 u_char *authenticator, char *buffer, int length)
{
	...
	req = grad_decode_pdu(host, udp_port, (u_char*) buffer, length);
	req->secret = secret;
...
grad_avp_t *
grad_client_encrypt_pairlist(grad_avp_t *plist, u_char *authenticator,
			     char *secret)
{
	grad_avp_t *p;
...
grad_avp_t *
grad_client_decrypt_pairlist(grad_avp_t *plist, u_char *authenticator,
			     char *secret)
{
	grad_avp_t *p;
		  ...
		  int *authid, u_char *authvec)
{
	...
	GRAD_DEBUG(1, "sending %s", grad_request_code_to_name(code));
	...
	do {
		...
		GRAD_DEBUG(10, "sending request (timeout=%u, retries=%u)",
			    config->timeout, config->retries);

		for (i = 0; i < config->retries; i++) {
			if (i)
				GRAD_DEBUG(10, "%s", "no response. retrying.");
...
grad_server_queue_t *
grad_client_create_queue(int read_cfg, uint32_t source_ip, size_t bufsize)
{
