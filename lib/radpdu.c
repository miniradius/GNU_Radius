...
grad_pdu_finish(void **ptr, struct radius_pdu *pdu,
		int code, int id, u_char *authenticator, char *secret)
{
...
grad_encode_pair(struct radius_attr *ap, grad_avp_t *pair)
{
	...
	case GRAD_TYPE_IPADDR:
		...
		rc = grad_attr_write(ap, &lval, sizeof(uint32_t));
		break;
...
grad_create_pdu(void **rptr, int code, int id, u_char *authenticator,
		char *secret, grad_avp_t *pairlist, char *msg)
{
	...
	for (pair = pairlist; pair; pair = pair->next) {
		...
		if ((vendorcode = GRAD_VENDOR_CODE(pair->attribute)) > 0
			...
			if (vendorpec == 429) {
				...
				uint32_t atval = htonl(pair->attribute & 0xffff);
...
static grad_avp_t *
grad_decode_pair(uint32_t attrno, char *ptr, size_t attrlen)
{
	...
	case GRAD_TYPE_STRING:
		...
		pair->avp_strvalue = grad_ebstrndup(ptr, attrlen);
	...
	case GRAD_TYPE_INTEGER:
	case GRAD_TYPE_IPADDR:
		memcpy(&lval, ptr, sizeof(uint32_t));
	...
	default:
		GRAD_DEBUG(1, "%s (Unknown Type %d)", attr->name,attr->type);
		grad_avp_free(pair);
		pair = NULL;
		break;
	}
...
static int
decode_vsa(u_char *ptr, uint32_t attrlen, uint32_t *vendorpec, uint32_t *vendorcode)
{
	uint32_t x;
...
grad_request_t *
grad_decode_pdu(uint32_t host, u_short udp_port, u_char *buffer, size_t length)
{
	u_char          *ptr;
	...
	while (ptr < endp && !stop) {
		uint32_t attrno, attrlen, vendorcode, vendorpec;

		attrno = *ptr++;
		attrlen = *ptr++;
		if (attrlen < 2) {
			GRAD_DEBUG(1, "%s", "exit from the loop");
			stop = 1;
			continue;
		}
		attrlen -= 2;
		...
		if (attrno == DA_VENDOR_SPECIFIC
			...
			while (attrlen > 0) {
				...
				pair = grad_decode_pair(attrno, (char*)ptr, len);
				if (pair) {
					if (first_pair == NULL)
						first_pair = pair;
					else
			...
			pair = grad_decode_pair(attrno, (char*)ptr, attrlen);
			ptr += attrlen;
			if (pair) {
				if (first_pair == NULL)
					first_pair = pair;
